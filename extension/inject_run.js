...
async function muiMultiSelect(comboboxEl, values) {
  ...
  // Close any stale open dropdown before we start
  document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', keyCode: 27, bubbles: true }));
  await sleep(300);

  // Open via React fiber onMouseDown, fallback to raw mousedown event
  function fireMouseDown(el) {
    ...
    if (fk) {
      ...
      while (fiber) {
        ...
        if (h) {
          h({ target: el, currentTarget: el, type: 'mousedown', button: 0, buttons: 1, preventDefault: () => {}, stopPropagation: () => {}, nativeEvent: { stopImmediatePropagation: () => {} } });
          return;
        }
        fiber = fiber.return;
      }
  ...
  fireMouseDown(comboboxEl);
  if (comboboxEl.parentElement) fireMouseDown(comboboxEl.parentElement);
  await sleep(700);

  if (!document.querySelector('[role="option"]')) {
    log('Warning: players dropdown did not open', 'error');
    return;
  }

  for (const val of values) {
    const opt = Array.from(document.querySelectorAll('[role="option"]'))
      .find(o => o.textContent.trim() === val);
    if (!opt) {
      log(`Warning: player "${val}" not found in dropdown`, 'error');
      continue;
    }
    // mousedown first prevents the autocomplete from losing focus before click fires
    opt.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, cancelable: true, view: window, button: 0, buttons: 1 }));
    await sleep(50);
    opt.click();
    await sleep(400);
  ...
  // Close via Escape (most reliable for MUI Select)
  comboboxEl.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', keyCode: 27, bubbles: true }));
  document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', keyCode: 27, bubbles: true }));
  await waitFor(() => !document.querySelector('[role="listbox"], #menu-selectedPlayer'), 3000).catch(() => {});
  await sleep(400);
}
...
async function selectMuiAutocomplete(comboEl, optionName) {
  ...
  // Select the matching option with mousedown+click (mousedown prevents autocomplete from blurring)
...
async function defineGroups(substageCard, teams) {
  ...
  for (const team of teams) {
    ...
    // Find the team autocomplete — last non-INPUT combobox in the modal (just added row)
    const dlg = document.querySelector('.MuiModal-root:not([aria-hidden="true"])') || document.body;
    const teamCombo = await waitFor(() =>
      Array.from(dlg.querySelectorAll('[role="combobox"]')).filter(el => el.tagName !== 'INPUT').pop()
    , 5000);

    log(`Selecting team "${team.name}"...`);
    const failed = await selectMuiAutocomplete(teamCombo, team.name).then(() => false).catch(async (e) => {
      log(`Warning: team "${team.name}" not found — skipping`, 'error');
      document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', keyCode: 27, bubbles: true }));
      await sleep(500);
      return true;
    });
    ...
    await sleep(3000); // wait for players field to render after team selection

    // Find the players combobox — prefer by ID, then last non-INPUT combobox
    const playersCombo = dlg.querySelector('#selectedPlayer') ||
      Array.from(dlg.querySelectorAll('[role="combobox"]')).filter(el => el.tagName !== 'INPUT').pop();

    const validPlayers = team.players.map(p => p.name).filter(n => n && n.trim());
    if (playersCombo && validPlayers.length) {
      await muiMultiSelect(playersCombo, validPlayers).catch(e => {
        log(`Warning: could not select players: ${e.message}`, 'error');
      });
    }
    await sleep(400);
  }
  ...
  // Close the Define Groups dialog via the X icon button at top right
  const dgModal = document.querySelector('.MuiModal-root:not([aria-hidden="true"])') || document.querySelector('[role="dialog"]');
  const closeBtn = Array.from((dgModal || document).querySelectorAll('button')).find(b => {
  ...
  if (closeBtn) {
    ...
    await waitFor(() => !document.querySelector('.MuiModal-root:not([aria-hidden="true"])'), 3000).catch(() => {});
    await sleep(300);
  } else {
    // Fallback: Escape key
    await closeAnyPanel();
...
function log(text, level = 'info') {
  if (window.__testLogs) window.__testLogs.push('['+level+'] '+text);
...
if (typeof chrome !== 'undefined' && chrome.runtime) chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  // Apply env-specific credentials if provided
  if (msg.creds) {
  ...
  if (msg.type === 'START_AUTOMATION') {
    if (_running) { sendResponse({ ok: false, reason: 'already running' }); return true; }
    _paused = false;
    _stopped = false;
    _running = true;
...
console.log('[TIP Automator] Content script loaded ✓');
  ...
  // Now call createSubstage with existing tournament+stage
  try {
    const id = await createSubstage('test-substage-playwright', 'ONE_V_ONE', 'Blaze Cup', 'Blaze Cup');
    window.__testResult = { ok: true, id };
  } catch(e) {
